...
class FoodItemAccessObjectTest {
	...
	void testCreateFoodItem() {
		...
		foodItem = new FoodItem("kokis", 2.5, true);
		assertEquals(false, foodItemDao.createFoodItem(foodItem), "couldn't create food item");
		assertEquals(2, foodItemDao.readFoodItems().length, "createFoodItem does not work!");
	...
	void testReadFoodItem() {
		...
		foodItem = new FoodItem("Iso kokis", 3.5, false);
		foodItemDao.createFoodItem(foodItem);
		assertEquals("kokis", foodItemDao.readFoodItem(foodItemDao.readFoodItemByName("kokis").getItemId()).getName(),
				"Couldn't read first item from list!");
		assertEquals("Iso kokis",
				foodItemDao.readFoodItem(foodItemDao.readFoodItemByName("Iso kokis").getItemId()).getName(),
				"Couldn't read last item from list!");
	...
	void testUpdateFoodItem() {
		...
		assertEquals(false, foodItemDao.readFoodItems()[0].isInMenu(),
				"Fooditems menu acitivity did not update correctyl!");
		assertEquals("None", foodItemDao.readFoodItems()[0].getCategory(),
				"Fooditems category did not update correctyl!");
	...
	void testGetByCategory() {
		...
		foodItemDao.createFoodItem(foodItem);
		assertEquals(2, foodItemDao.readFoodItemsCategory("juomat").length,
				"Couldn't find all items from category juomat");
		assertEquals(1, foodItemDao.readFoodItemsCategory("hampurilaiset").length,
				"Couldn't find all items from category hampurilaiset");
		assertEquals(1, foodItemDao.readFoodItemsCategory("None").length, "Couldn't find all items from category None");
		assertEquals(0, foodItemDao.readFoodItemsCategory("Isot Juomat").length,
	...
	void testDeleteFoodItem() {
		...
		foodItemDao.createFoodItem(foodItem);
		assertEquals(2, foodItemDao.readFoodItems().length, "read all food items does not work!(wrong size)");
		assertEquals(true, foodItemDao.deleteFoodItem(foodItemDao.readFoodItemByName("kokis").getItemId()),
				"Deleting item doesn't return true!");
	...
	void testIngredients() {
		...
		foodItemDao.createFoodItem(foodItem);
		assertEquals(4, foodItemDao.readFoodItemByName("juustohampurilainen").getIngredientsAsList().length,
				"Couldn't get or set ingredients to db");
		assertEquals("Sämpylä", foodItemDao.readFoodItemByName("juustohampurilainen").getIngredientsAsList()[0],
				"Couldn't get or set ingredients to db");
		assertEquals("Pihvi", foodItemDao.readFoodItemByName("juustohampurilainen").getIngredientsAsList()[1],
				"Couldn't get or set ingredients to db");
		assertEquals("juusto", foodItemDao.readFoodItemByName("juustohampurilainen").getIngredientsAsList()[2],
				"Couldn't get or set ingredients to db");
		assertEquals("ketsuppi", foodItemDao.readFoodItemByName("juustohampurilainen").getIngredientsAsList()[3],
				"Couldn't get or set ingredients to db");
	}
	...
	void testReadByName() {
		...
		assertEquals(5, foodItemDao.readFoodItemByName("Superhampurilainen").getPrice(),
				"Couldn't find superhampurilainen");
